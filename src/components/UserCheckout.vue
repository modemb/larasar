...
import { cy, i18n, xRate } from 'boot/axios'
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const getCurrencies = computed(() => $store.getters['users/currenciesGetter'])

    const currency = ref(auth.value?.currency_code)
    ...
    watch(currency, currency => {
      ...
      // crudAction({ rate: null, mutate: 'rateMutation' })
      // crudAction({ currency, mutate: 'currencyMutation' })

      xRate(auth.value); setTimeout(() => emit('currency'), 1500)
    ...
    return {
      ...
      filterFn (val, update) { // ===== TagChooseCurrency ================= \\
        update(async () => {
          const needle = val.toLowerCase()
          if (!getCurrencies.value?.length) crudAction({
              url: 'api/users/currencies', method: 'get',
              currenciesData: 'currencies'
            }).then(currencies => $store.commit('users/currenciesMutation', { currencies }))
